...
- Outcome variable
$$y = (y_{1},\ldots,y_{K})$$ 
where $y_{k}=1$ if the outcome belongs to the $k^{\text{th}}$ class, and zero otherwise. 

- Conditional probability
$$p_{k}\equiv\operatorname{Pr}\left(y_{k}=1 | \mathbf{x}_{}\right)$$
In words: the $p_{k}$ is the probability that $y$ belongs to class $k$, given $\mathbf{x}_i$.

---
# Multinomial regression model

For each class $k=1,\ldots,K$, we model the outcome as
...
$$\min _{\left(\beta_{0}, \beta\right) \in \mathbb{R}^{p+1}} \left\{-\frac{2}{N} \sum_{i=1}^N \sum_{k=1}^K y_{i k} \log p_{i k}+\lambda \sum_{i=1}^p \sum_{k=1}^K \left|\beta_{k j}\right|\right\}$$
